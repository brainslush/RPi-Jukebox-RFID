...
from .models import AlarmActiveState, AlarmConfig, AlarmId, AlarmVolumeConstantConfig, AlarmVolumeFadeInConfig, CardId
...
class AlarmExecutor:
    ...
    def __init__(self,
        ...
        self._pre_alarm_volume: int | None = None
    ...
    def fire(self):
        ...
        self._pre_alarm_volume = self._get_volume()
        ...
        match(vol):
            case(
                AlarmVolumeFadeInConfig(
                    start=start, target=target, ramp_seconds=ramp_seconds, steps=steps
                )
            ):
                ...
                self._fade_in.start()
            case(AlarmVolumeConstantConfig(volume=volume)):
    ...
    def dismiss(self):
        ...
        if self._cfg.volume.restore_after and self._pre_alarm_volume is not None:
            self._set_volume(self._pre_alarm_volume)
